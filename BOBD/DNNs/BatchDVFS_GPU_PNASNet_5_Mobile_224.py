...
##    dummy_input = np.random.random_sample((batch_size,224,224,3))
  ...
  with tf.Session(graph=g,config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      for k in range(num_iters):
        ...
        printLables = 0
        if printLables == 1:
            if os.path.exists('resultLables_PNASNet_5_Mobile_224.txt'):
                append_write = 'a' # append if already exists
            else:
                append_write = 'w' # make a new file if not
            #
            highscore = open('resultLables_PNASNet_5_Mobile_224.txt',append_write)
      ...
      # if os.path.exists('runtimes_PNASNet_5_Mobile_224.txt'):
      #     append_write = 'a' # append if already exists
      # else:
      #     append_write = 'w' # make a new file if not
      #
      # runtimeResults = open('runtimes_PNASNet_5_Mobile_224.txt',append_write)
