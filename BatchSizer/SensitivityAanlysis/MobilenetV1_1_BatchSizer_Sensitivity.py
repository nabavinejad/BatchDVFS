...
##    dummy_input = np.random.random_sample((batch_size,224,224,3))
  ...
  with tf.Session(graph=g,config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      while True:
          ...
          if (time.time() - power_cap_change) > 40 and powerCapW < 175:
              powerCapW = powerCapW + 50
              power_cap_change = time.time()
          ...
          if (time.time() - two_second_start) > 3 :
              ...
              if max(powerReading) <= powerCapW and max(powerReading) >=  (0.85 * powerCapW):
                  powerReading[:] = []
                  pass

              elif max(powerReading) < (0.85 * powerCapW):   # Power less than power cap
                  powerReading[:] = []
                  if BestBS == MAXINDEX:
...
if "__main__" in __name__:
  ...
  if f.native:
    ...
    if os.path.exists('runtimes_MobilenetV1_1_BS.txt'):
        append_write = 'a'  # append if already exists
    else:
        append_write = 'w'  # make a new file if not

    runtimeResults = open('runtimes_MobilenetV1_1_BS.txt', append_write)
