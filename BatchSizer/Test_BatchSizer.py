...
def readPower():
    for i in range(250):
            os.system("nvidia-smi --query-gpu=power.draw --format=csv,noheader,nounits -f powerDraw.txt")
            with open("powerDraw.txt", "r") as file:
                for last_line in file:
                    pass
            powerReading.append(float(last_line))
            completePowerReading.append(float(last_line))
            time.sleep(0.3)
...
##    dummy_input = np.random.random_sample((batch_size,224,224,3))
  ...
  with tf.Session(graph=g,config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      while True:
          ...
          if (time.time() - two_second_start) > 2 :
          #batchCounter = batchCounter + 1
          #if batchCounter == 10:
              batchCounter = 0
              two_second_start = time.time()
              # resultsFile.write(
              #     str(time.time()) + "," + str(ThroughputPerSecond[-1]) + "," + str(powerReading[-1]) + "," + str(
              #         newbatchSize)
              ...
              if max(powerReading) <= powerCapW and max(powerReading) >=  (0.8 * powerCapW):
                  ...
                  powerReading[:] = []
                  if True:
                  #if currentDVFS == maxDVFS:    # DVFS cannot help anymore, need to increase the batch size
                      if BestBS == MAXINDEX:
                  ...
                  powerReading[:] = []
                  if True:
                  #if currentDVFS == minDVFS:   # DVFS cannot help anymore, need to increase the batch size
                      if BestBS == MININDEX:
...
if "__main__" in __name__:
  ...
  if f.native:
    ...
    endTime = time.time()
    os.system('sudo pkill nvidia-smi')
    c.terminate()
    t.join()


    if os.path.exists('runtimes_InceptionV1_BS.txt'):
        append_write = 'a'  # append if already exists
    else:
        append_write = 'w'  # make a new file if not

    runtimeResults = open('runtimes_InceptionV1_BS.txt', append_write)
